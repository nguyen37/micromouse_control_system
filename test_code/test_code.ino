...
double RH_dist_val = 0;
double LH_dist_val = 0;
double CN_dist_val = 0;
...
void loop() {
  ...
  Serial.print("center: ");
  Serial.print(CN_dist_val);
  Serial.print("right: ");
  Serial.println(RH_dist_val);
  */
  // Wait some time
  float end_d = millis();
  float delay_amount = end_d - start;
...
void get_dist() {
  ...
  */
  RH_dist_val = 1/analogRead(RH_IR_pin);
  LH_dist_val = analogRead(LH_IR_pin);
  CN_dist_val = analogRead(CN_IR_pin);
